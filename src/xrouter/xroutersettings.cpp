...
std::string XRouterSettings::host(XRouterCommand c, const std::string & service) {
    ...
    // TODO ReptiloidsCoin XRouter support subsection host designations
...
int XRouterSettings::port(XRouterCommand c, const std::string & service) {
    ...
    // TODO ReptiloidsCoin XRouter support subsection port designations
//    if (c == xrService) { // Handle plugin
//        if (!service.empty())
...
bool XRouterSettings::tls(XRouterCommand c, const std::string & service) {
    ...
    // TODO ReptiloidsCoin XRouter support subsection tls designations
    ...
    try {
        ...
        if (!boost::filesystem::exists(p)) {
            saveConf(p,
                     ...
                     "tls=0"                                                                                             + eol +
                     ""                                                                                                  + eol +
                     "#! maxfee is the maximum fee (in REPT) you're willing to pay on a single xrouter call"            + eol +
        ...
        if (!skipPlugins && !boost::filesystem::exists(plugins)) {
            ...
            saveConf(samplerpc,
                     ...
                     "parameters="                                                                                       + eol +
                     ""                                                                                                  + eol +
                     "#! Set the fee in REPT to how much you want to charge for requests to this custom plugin."        + eol +
                     "#! Example fee=0.1 if you want to accept 0.1 REPT or 0 if you want the plugin to be free."        + eol +
                     "fee=0"                                                                                             + eol +
                     ""                                                                                                  + eol +
