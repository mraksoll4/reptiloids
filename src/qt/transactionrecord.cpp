            ...
            if(mine)
            {
                ...
                if (wtx.is_coinbase)
                {
                    ...
                    if (wtx.tx->vout.size() > 2) { // if stake-to-address
                        ...
                        // Generated
                        sub.type = TransactionRecord::Generated;
                        sub.debit = -wtx.debit; // ReptiloidsCoin track debit amount for coinstakes
                    }
                }
...
void TransactionRecord::updateStatus(const interfaces::WalletTxStatus& wtx, int numBlocks, int64_t block_time)
{
    ...
    const bool up_to_date = ((int64_t)QDateTime::currentMSecsSinceEpoch() / 1000 - block_time < MAX_REPT_TIME_GAP);
