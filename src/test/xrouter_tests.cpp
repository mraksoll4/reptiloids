...
BOOST_FIXTURE_TEST_CASE(xrouter_tests_testnet_xrcalls, XRouterTestClientTestnet) {
    BOOST_REQUIRE_MESSAGE(client->start(), "Failed to start xrouter client");
    BOOST_REQUIRE_MESSAGE(client->waitForService(5000, "xr::REPT"), "Services not found on testnet");
    ...
    { // getBlockCount
        UniValue res = client->getBlockCount("REPT");
    ...
    { // getBlockCount with namespace
        UniValue res = client->getBlockCount("xr::REPT");
        auto replynull = find_value(res, "reply").isNull();
        BOOST_CHECK_MESSAGE(!replynull, "getBlockCount reply is invalid");
        if (!replynull)
            BOOST_CHECK_MESSAGE(find_value(res, "reply").isNum(), "getBlockCount reply is not a number");
    ...
    { // getBlockHash
        UniValue res = client->getBlockHash("REPT", 1);
        auto replynull = find_value(res, "reply").isNull();
        ...
        // TODO ReptiloidsCoin libxrouter hex number
//        res = client->getBlockHash("REPT", "0x1");
...
//        BOOST_CHECK_MESSAGE(find_value(res, "error").isNull(), "getBlockHash has error");
    ...
    { // getBlock
        // TODO ReptiloidsCoin XRouter int block
//        UniValue res = client->getBlock("REPT", 1);
//        auto replynull = find_value(res, "reply").isNull();
...
//        BOOST_CHECK_MESSAGE(find_value(res, "error").isNull(), "getBlock has error");
        ...
        // string block
        auto res = client->getBlock("REPT", testnetBlock1Hash);
        auto replynull = find_value(res, "reply").isNull();
        BOOST_CHECK_MESSAGE(!replynull, "getBlock reply is invalid");
        if (!replynull) {
            BOOST_CHECK_MESSAGE(find_value(res, "reply").isObject(), "getBlock reply is not a json obj");
    ...
    { // getBlocks
        // TODO ReptiloidsCoin XRouter int block numbers
//        UniValue res = client->getBlocks("REPT", {1, 2});
//        auto replynull = find_value(res, "reply").isNull();
...
//        BOOST_CHECK_MESSAGE(find_value(res, "error").isNull(), "getBlocks has error");
        ...
        // string block hashes
        auto res = client->getBlocks("REPT", {testnetBlock1Hash, testnetBlock2Hash});
        auto replynull = find_value(res, "reply").isNull();
        BOOST_CHECK_MESSAGE(!replynull, "getBlocks reply is invalid");
        if (!replynull) {
            auto reply = find_value(res, "reply");
    ...
    { // getTransaction
        auto res = client->getTransaction("REPT", testnetBlock1Tx);
        auto replynull = find_value(res, "reply").isNull();
    ...
    { // getTransactions
        UniValue res = client->getTransactions("REPT", {testnetBlock1Tx, testnetBlock2Tx});
        auto replynull = find_value(res, "reply").isNull();
    ...
    { // decodeTransaction
        auto res = client->decodeTransaction("REPT", testnetBlock1TxRaw);
        auto replynull = find_value(res, "reply").isNull();
    ...
    { // sendTransaction
        auto res = client->sendTransaction("REPT", testnetBlock1TxRaw);
...
BOOST_FIXTURE_TEST_CASE(xrouter_tests_testnet_xrservicecalls, XRouterTestClientTestnet) {
    BOOST_REQUIRE_MESSAGE(client->start(), "Failed to start xrouter client");
    BOOST_REQUIRE_MESSAGE(client->waitForService(5000, {{"xr::REPT",1},{"xrs::block_getblockhash",1}}), "Services not found on testnet");
    BOOST_REQUIRE_MESSAGE(!client->getServiceNodes().empty(), "Service node list should not be empty");
...
/*BOOST_FIXTURE_TEST_CASE(xrouter_tests_mainnet_xrcalls, XRouterTestClientMainnet) {
        BOOST_REQUIRE_MESSAGE(client->start(), "Failed to start xrouter client");
        BOOST_REQUIRE_MESSAGE(client->waitForService(10000, "xr::REPT"), "Services not found on mainnet");
        BOOST_REQUIRE_MESSAGE(!client->getServiceNodes().empty(), "Service node list should not be empty");
        ...
        { // getBlockHash
            ...
            BOOST_CHECK_MESSAGE(find_value(res, "error").isNull(), "getBlockHash has error");
            // hex number
//        res = client->getBlockHash("REPT", "0x01");
//        replynull = find_value(res, "reply").isNull();
//        BOOST_CHECK_MESSAGE(!replynull, "getBlockHash reply is invalid");
//        if (!replynull) {
//            BOOST_CHECK_MESSAGE(find_value(res, "reply").isStr(), "getBlockHash reply is not a string");
