...
BOOST_AUTO_TEST_CASE(versionbits_computeblockversion)
{
    ...
    lastBlock = firstChain.Mine(mainnetParams.nMinerConfirmationWindow, nTime, VERSIONBITS_LAST_OLD_REPT_VERSION).Tip();
    BOOST_CHECK_EQUAL(ComputeBlockVersion(lastBlock, mainnetParams) & (1<<bit), 0);

    // Mine 2011 more blocks at the old time, and check that CBV isn't setting the bit yet.
    for (int i=1; i<mainnetParams.nMinerConfirmationWindow-4; i++) {
        lastBlock = firstChain.Mine(mainnetParams.nMinerConfirmationWindow+i, nTime, VERSIONBITS_LAST_OLD_REPT_VERSION).Tip();
        // This works because VERSIONBITS_LAST_OLD_REPT_VERSION happens
    ...
    nTime = nStartTime;
    for (int i=mainnetParams.nMinerConfirmationWindow-4; i<=mainnetParams.nMinerConfirmationWindow; i++) {
        lastBlock = firstChain.Mine(mainnetParams.nMinerConfirmationWindow+i, nTime, VERSIONBITS_LAST_OLD_REPT_VERSION).Tip();
        BOOST_CHECK_EQUAL(ComputeBlockVersion(lastBlock, mainnetParams) & (1<<bit), 0);
    }

    // Advance to the next period and transition to STARTED,
    lastBlock = firstChain.Mine(mainnetParams.nMinerConfirmationWindow*3, nTime, VERSIONBITS_LAST_OLD_REPT_VERSION).Tip();
    ...
    while (nTime < nTimeout && blocksToMine > 0) {
        lastBlock = firstChain.Mine(nHeight+1, nTime, VERSIONBITS_LAST_OLD_REPT_VERSION).Tip();
        BOOST_CHECK((ComputeBlockVersion(lastBlock, mainnetParams) & (1<<bit)) != 0);
        BOOST_CHECK_EQUAL(ComputeBlockVersion(lastBlock, mainnetParams) & VERSIONBITS_TOP_MASK, VERSIONBITS_TOP_BITS);
        blocksToMine--;
        nTime += 600;
    ...
    for (int i=0; i<mainnetParams.nMinerConfirmationWindow-1; i++) {
        lastBlock = firstChain.Mine(nHeight+1, nTime, VERSIONBITS_LAST_OLD_REPT_VERSION).Tip();
        BOOST_CHECK((ComputeBlockVersion(lastBlock, mainnetParams) & (1<<bit)) != 0);
        nHeight += 1;
    }
    ...
    // next period.
    lastBlock = secondChain.Mine(mainnetParams.nMinerConfirmationWindow, nTime, VERSIONBITS_LAST_OLD_REPT_VERSION).Tip();
    BOOST_CHECK((ComputeBlockVersion(lastBlock, mainnetParams) & (1<<bit)) != 0);
    ...
    lastBlock = secondChain.Mine(mainnetParams.nMinerConfirmationWindow*3-1, nTime, VERSIONBITS_LAST_OLD_REPT_VERSION).Tip();
    BOOST_CHECK((ComputeBlockVersion(lastBlock, mainnetParams) & (1<<bit)) != 0);
    lastBlock = secondChain.Mine(mainnetParams.nMinerConfirmationWindow*3, nTime, VERSIONBITS_LAST_OLD_REPT_VERSION).Tip();
    BOOST_CHECK_EQUAL(ComputeBlockVersion(lastBlock, mainnetParams) & (1<<bit), 0);

    // Finally, verify that after a soft fork has activated, CBV no longer uses
    // VERSIONBITS_LAST_OLD_REPT_VERSION.
    //BOOST_CHECK_EQUAL(ComputeBlockVersion(lastBlock, mainnetParams) & VERSIONBITS_TOP_MASK, VERSIONBITS_TOP_BITS);
}
