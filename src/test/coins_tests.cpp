...
BOOST_AUTO_TEST_CASE(updatecoins_simulation_test)
{
    ...
    for (unsigned int i = 0; i < NUM_SIMULATION_ITERATIONS; i++) {
        ...
        if (randiter % 20 < 19) {
            ...
            if (randiter % 20 < 2 || coinbase_coins.size() < 10) {
                ...
                /*if (InsecureRandRange(10) == 0 && coinbase_coins.size()) { // ReptiloidsCoin PoS no duplicate coinbases
    ...
    // Verify coverage.
//    BOOST_CHECK(spent_a_duplicate_coinbase); // ReptiloidsCoin PoS no duplicate coinbases
}
