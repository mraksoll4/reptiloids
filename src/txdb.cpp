...
static const char DB_REPT_FILES = 'f';
static const char DB_REPT_INDEX = 'b';

static const char DB_BEST_REPT = 'B';
static const char DB_HEAD_REPTS = 'H';
static const char DB_FLAG = 'F';
static const char DB_REINDEX_FLAG = 'R';
static const char DB_LAST_REPT = 'l';
...
uint256 CCoinsViewDB::GetBestBlock() const {
    ...
    if (!db.Read(DB_BEST_REPT, hashBestChain))
        return uint256();
...
std::vector<uint256> CCoinsViewDB::GetHeadBlocks() const {
    ...
    if (!db.Read(DB_HEAD_REPTS, vhashHeadBlocks)) {
...
bool CCoinsViewDB::BatchWrite(CCoinsMap &mapCoins, const uint256 &hashBlock) {
    ...
    batch.Erase(DB_BEST_REPT);
    batch.Write(DB_HEAD_REPTS, std::vector<uint256>{hashBlock, old_tip});
    ...
    batch.Erase(DB_HEAD_REPTS);
    batch.Write(DB_BEST_REPT, hashBlock);
...
bool CBlockTreeDB::ReadBlockFileInfo(int nFile, CBlockFileInfo &info) {
    return Read(std::make_pair(DB_REPT_FILES, nFile), info);
}
...
bool CBlockTreeDB::ReadLastBlockFile(int &nFile) {
    return Read(DB_LAST_REPT, nFile);
}
...
bool CBlockTreeDB::WriteBatchSync(const std::vector<std::pair<int, const CBlockFileInfo*> >& fileInfo, int nLastFile, const std::vector<const CBlockIndex*>& blockinfo) {
    ...
    for (std::vector<std::pair<int, const CBlockFileInfo*> >::const_iterator it=fileInfo.begin(); it != fileInfo.end(); it++) {
        batch.Write(std::make_pair(DB_REPT_FILES, it->first), *it->second);
    }
    batch.Write(DB_LAST_REPT, nLastFile);
    for (std::vector<const CBlockIndex*>::const_iterator it=blockinfo.begin(); it != blockinfo.end(); it++) {
        batch.Write(std::make_pair(DB_REPT_INDEX, (*it)->GetBlockHash()), CDiskBlockIndex(*it));
    }
...
bool CBlockTreeDB::LoadBlockIndexGuts(const Consensus::Params& consensusParams, std::function<CBlockIndex*(const uint256&)> insertBlockIndex)
{
    ...
    pcursor->Seek(std::make_pair(DB_REPT_INDEX, uint256()));
            ...
            std::vector<CDiskBlockIndex> & blocks, std::unordered_set<uint256, TXDBHasher> & invalidBlocks) -> bool
    {
        ...
        for (int i = 0; i < cores; ++i) {
            ...
            tg.create_thread([from,to,&blocks,&invalidBlocks,&insertBlockIndex,&mu,&progress,totalprogress,estTotalBlocks,consensusParams,this] {
                RenameThread("reptiloidscoin-blockindex");
    ...
    while (pcursor->Valid()) {
        ...
        if (pcursor->GetKey(key) && key.first == DB_REPT_INDEX) {
