...
static void AssembleBlock(benchmark::State& state)
{
    ...
    constexpr size_t NUM_REPTS{50}; // ReptiloidsCoin uses Quark hash, keep this smaller b/c it's much more cpu intensive
    std::vector<CTransactionRef> txs; txs.resize(NUM_REPTS - chainparams.GetConsensus().coinMaturity + 1);
    for (size_t b{0}; b < NUM_REPTS; ++b) {
        CMutableTransaction tx;
        tx.vin.push_back(MineBlock(SCRIPT_PUB));
        tx.vin.back().scriptWitness = witness;
        tx.vout.emplace_back(1337, SCRIPT_PUB);
        if (NUM_REPTS - b >= chainparams.GetConsensus().coinMaturity)
