...
static const unsigned int MAX_REPTFILE_SIZE = 0x8000000; // 128 MiB
/** The pre-allocation chunk size for blk?????.dat files (since 0.8) */
static const unsigned int REPTFILE_CHUNK_SIZE = 0x1000000; // 16 MiB
...
static const int MAX_REPTS_IN_TRANSIT_PER_PEER = 64;
/** Timeout in seconds during which a peer must stall block download progress before being disconnected. */
static const unsigned int REPT_STALLING_TIMEOUT = 2;
/** Number of headers sent in one getheaders result. We rely on the assumption that if a peer sends
 *  less than this number, we reached its tip. Changing this value is a protocol upgrade. */
static const unsigned int MAX_HEADERS_RESULTS = 2000;
/** Maximum depth of blocks we're willing to serve as compact blocks to peers
 *  when requested. For older blocks, a regular REPT response will be sent. */
static const int MAX_CMPCTREPT_DEPTH = 5;
/** Maximum depth of blocks we're willing to respond to GETREPTTXN requests for. */
static const int MAX_REPTTXN_DEPTH = 10;
/** Size of the "block download window": how far ahead of our current height do we fetch?
 *  Larger windows tolerate larger download speed differences between peer, but increase the potential
 *  degree of disordering of blocks on disk (which make reindexing and pruning harder). We'll probably
 *  want to make this a per-peer adaptive value at some point. */
static const unsigned int REPT_DOWNLOAD_WINDOW = 1024;
/** Time to wait (in seconds) between writing blocks/block index to disk. */
static const unsigned int DATABASE_WRITE_INTERVAL = 60 * 10; // every 10 min
/** Time to wait (in seconds) between flushing chainstate to disk. */
static const unsigned int DATABASE_FLUSH_INTERVAL = 60 * 60; // every hour
/** Maximum length of reject messages. */
static const unsigned int MAX_REJECT_MESSAGE_LENGTH = 111;
/** Block download timeout base, expressed in millionths of the block interval (i.e. 10 min) */
static const int64_t REPT_DOWNLOAD_TIMEOUT_BASE = 1000000;
/** Additional block download timeout per parallel downloading peer (i.e. 5 min) */
static const int64_t REPT_DOWNLOAD_TIMEOUT_PER_PEER = 500000;

static const int64_t DEFAULT_MAX_TIP_AGE = 6 * 60 * 60; // 6 hours
/** Maximum age of our tip in seconds for us to be considered current for fee estimation */
static const int64_t MAX_FEE_ESTIMATION_TIP_AGE = 20 * 60; // 20 minutes
...
static const unsigned int MAX_REPTS_TO_ANNOUNCE = 8;
...
extern std::map<int, CBlockIndex*> mapHeaderIndex GUARDED_BY(cs_main);
...
/** Block files containing a block-height within MIN_REPTS_TO_KEEP of chainActive.Tip() will not be pruned. */
static const unsigned int MIN_REPTS_TO_KEEP = 288;
/** Minimum blocks required to signal NODE_NETWORK_LIMITED */
static const unsigned int NODE_NETWORK_LIMITED_MIN_REPTS = 288;

static const signed int DEFAULT_CHECKREPTS = 100;
...
static const uint64_t MIN_DISK_SPACE_FOR_REPT_FILES = 550 * 1024 * 1024;
...
{
    return (fHavePruned && !(pblockindex->nStatus & REPT_HAVE_DATA) && pblockindex->nTx > 0);
}
 ...
 * @return bool
 */
static const int SNODE_STALE_REPTS = 10; // number of blocks to allow before a snode is marked "stale"
