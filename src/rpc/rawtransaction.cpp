...
static UniValue getrawtransaction(const JSONRPCRequest& request)
{
    const RPCHelpMan help{
            ...
            "           \"address\"        (string) reptiloidscoin address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
...
static UniValue decoderawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "         \"type\" : \"pubkeyhash\",  (string) The type, eg 'pubkeyhash'\n"
            "         \"addresses\" : [           (json array of string)\n"
            "           \"12tvKAXCxZjSmdNbao16dKXC8tRWfcF5oc\"   (string) reptiloidscoin address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            RPCHelpMan{"decodepsbt",
                "\nReturn a JSON object representing the serialized, base64-encoded partially signed ReptiloidsCoin transaction.\n",
                {
            ...
            "          \"hex\" : \"hex\",            (string) The hex\n"
            "          \"type\" : \"pubkeyhash\",    (string) The type, eg 'pubkeyhash'\n"
            "          \"address\" : \"address\"     (string) ReptiloidsCoin address if there is one\n"
            "        }\n"
            "      },\n"
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            RPCHelpMan{"combinepsbt",
                "\nCombine multiple partially signed ReptiloidsCoin transactions into one transaction.\n"
